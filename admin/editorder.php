...
if (!isset($_SESSION['admin_login'])) {
	...
	if (isset($_REQUEST['eoid'])) {
		...
		$eoid = $con->quote($_REQUEST['eoid']);
		$getposts5 = $con->query("SELECT * FROM orders WHERE id='$eoid'");
			if ($getposts5->rowCount()){
	...
	$result1 = $con->query("SELECT * FROM orders WHERE id='$eoid'");
		$get_order_info = $result1->fetch(PDO::FETCH_ASSOC);
			...
			$result2 = $con->query("SELECT * FROM user WHERE id='$eouid'");
			$get_order_info2 = $result2->fetch(PDO::FETCH_ASSOC);
			$euname = $get_order_info2['firstName'];
			$euemail = $get_order_info2['email'];
			$eumobile = $get_order_info2['mobile'];
}

$getposts = $con->query("SELECT * FROM products WHERE id ='$eopid'");
	...
	try {
		if(empty($_POST['dstatus'])) {
			throw new Exception('Status can not be empty');
				...
				if($con->query("UPDATE orders SET dstatus='$eodstatus', ddate='$ddate', quantity='$dquantity' WHERE id='$eoid'")){
					//success message
...
//triming name
	if($con->query("DELETE FROM orders WHERE id='$eoid'")){
