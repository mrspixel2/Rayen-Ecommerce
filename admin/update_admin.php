...
$_POST['last_name'] = trim($_POST['last_name']);
	try {
		...
		$check = 0;
		$e_check = $con->query("SELECT email FROM `admin` WHERE email='$u_email'");
		$email_check = $e_check->rowCount();
		if (strlen($_POST['first_name']) >2 && strlen($_POST['first_name']) <16 ) {
			if ($check == 0 ) {
				if (strlen($_POST['password']) >4 ) {
					...
					if($utype_db == 'admin'){
						$result = $con->query("UPDATE admin SET firstName='$_POST[first_name]', lastName='$_POST[last_name]', email='$u_email', mobile='$u_mobile', address='$u_address', password='$_POST[password]', type='$u_admin_type' WHERE id='$user'");
					}else{
						$result = $con->query("UPDATE admin SET firstName='$_POST[first_name]', lastName='$_POST[last_name]', email='$u_email', mobile='$u_mobile', address='$u_address', password='$_POST[password]' WHERE id='$user'");
					}
