...
if (isset($_POST['login'])) {
	if (isset($_POST['email']) && isset($_POST['password'])) {
		...
		$result = $con->prepare("select * from admin where (email=:user_login) and password=:password_login_md5");
		$result->bindParam('user_login', $user_login);
      	$result->bindValue('password_login_md5', $password_login_md5);
		$result->execute();
		echo $user_login." ".$password_login_md5;
		$num = $result->rowCount();
