...
$getposts = $con->query("select * from products WHERE id =".$pid);
					if ($getposts->rowCount()) {
						$row = $row = $getposts->fetch(PDO::FETCH_ASSOC);
						$id = $row['id'];
						$pName = $row['pName'];
						$price = $row['price'];
						$piece=$row['piece'];
...
if (isset($_POST['addcart'])) {
	$getposts = $con->prepare("select * from cart where pid = :pid and uid= :user");
	$getposts->bindParam('pid',$pid, PDO::PARAM_INT);
	$getposts->bindParam('user',$user, PDO::PARAM_INT);
	$getposts->execute();
	if ($getposts->rowCount()) {
		header('location: ../mycart.php?uid='.$user.'');
	}else{
		$getposts = $con->prepare("insert into cart (uid,pid,quantity) VALUES (:user,:pid, 1)");
		$getposts->bindParam('user',$user, PDO::PARAM_INT);
		$getposts->bindParam('pid',$pid, PDO::PARAM_INT);
		if($getposts->execute()){
			header('location: ../mycart.php?uid='.$user.'');
