...
$getposts = $con->prepare("select * from products WHERE id =:poid");
$getposts->bindParam('poid',$poid, PDO::PARAM_INT);
$getposts->execute();
...
//triming name
	try {
		...
		// Check if email already exists
						...
						$dd = date( "Y-m-d", strtotime( "$d +7 day" ) );
		...
		echo $user." ".$poid." ".$quan." ".$_POST['address']." ".$_POST['mobile']." ".$d." ".$dd." ".$del;
	}
}
...
<body style="background-image: url(image/homebackgrndimg1.jpg);">
	...
	$result = $con->query("select * from user WHERE id=".$user);
		...
		<div style="float: left; font-size: 23px;">
			<div>
				<?php
				$product = $con->prepare("select * from products WHERE id =:poid");
				$product->bindParam('poid',$poid, PDO::PARAM_INT);
				$product->execute();
										...
										$picturee = $row['picture'];
										$itemm = $row['item'];
										$categoryy = $row['category'];
										$availablee =$row['available'];
					echo '
						<ul style="float: left;">
							<li style="float: left; padding: 0px 25px 25px 25px;">
								<div class="home-prodlist-img"><a href="'.$categoryy.'/view_product.php?pid='.$idd.'">
									<img src="image/product/'.$itemm.'/'.$picturee.'" class="home-prodlist-imgi">
									</a>
									<div style="text-align: center; padding: 0 0 6px 0;"> <span style="font-size: 15px;">'.$pNamee.'</span><br> Price: '.$pricee.' Php</div>
								</div>
